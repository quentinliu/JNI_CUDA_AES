...
void f4printBytes(BYTE b[], int len, char *buff) {
int i;
int l;
l=strlen(buff);
char szTmp[3];
printf("len:%d",l);
for (i=0; i<len; i++){
   sprintf( szTmp, "%02x", (unsigned char) b[i] );
   memcpy( &buff[l + i * 2], szTmp, 2 );

   }
//   sprintf(buff, "%02x", b[i]);
//    cout << hex << b[i] << " " ;
}
...
JNIEXPORT jstring JNICALL Java_com_gputest_aesByGPU_testTwo
  (JNIEnv *env, jobject o, jstring src, jstring jkey){
//  (JNIEnv *env, jobject o, jstring s){
    const char* szSrc = (env)->GetStringUTFChars(src,0);
    const char* szKey = (env)->GetStringUTFChars(jkey,0);
...
*/
   ...
   char inStr[] = "123456789012345";
   char keyStr[] = "1234567890123456";
   printf("The in message: %s\n", inStr );
   int infileLength = strlen(inStr);
   printf("The in message length: %d\n", infileLength );
...
for(int i=0; i<block_number; i++){
    ...
    for(int j=0; j<16; j++){
        aes_block_array[i].block[j] = (unsigned char)inStr[i*16+j];
    }
}
if(number_of_zero_pending != 0)
{
    for(int j=0; j<16; j++){
        aes_block_array[block_number].block[j] = (unsigned char)inStr[block_number*16+j];
    }
...
for(int i=0; i<block_number-1; i++){
    f4printBytes(aes_block_array[i].block, blockLen, buffer);
}
if(number_of_zero_pending == 0)
    f4printBytes(aes_block_array[block_number-1].block, blockLen, buffer);
else 
    f4printBytes(aes_block_array[block_number-1].block, blockLen, buffer);

printf("After ecryption:%s\n",buffer);
...
AES_Done();
    ...
    (env)->ReleaseStringUTFChars(src,szSrc);
    (env)->ReleaseStringUTFChars(jkey,szKey);
